document.addEventListener('DOMContentLoaded', function () {
    ...
    if (typeof L === 'undefined') {
        console.error('Itinerary Route Map: Leaflet is not loaded.');
        return;
    }
    ...
    try {
        ...
        console.error('Itinerary Route Map: invalid stops data.', error);
        return;
    }
    ...
    function addMarkers() {
        ...
        points.forEach(function (point, index) {
            let popup = '<div class="irm-popup"><strong>Day ' + point.dayNumber + '</strong>';
    ...
    if (bounds.length > 1) {
        ...
        fetch('https://router.project-osrm.org/route/v1/driving/' + osrmCoords + '?overview=full&geometries=geojson')
            ...
            .catch(function (error) {
                console.warn('Itinerary Route Map: using fallback route line.', error);
                addMarkers();
            });
    } else {
        fitToBounds();
